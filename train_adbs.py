...
from models.utils_simgcd_adbs import DINOHead, get_params_groups, SupConLoss, info_nce_logits, DistillLoss
...
from models.utils_simgcd_adbs import InterClassConstraintLoss
...
def train_online(student, student_pre, proto_aug_manager, train_loader, test_loader, current_session, args):
    ...
    for epoch in range(args.epochs_online_per_session):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            # 手动将旧类别的边界权重梯度设为0（从第二个session开始）
            if current_session > 1:
                with torch.no_grad():
                    if student[1].boundary_weights.grad is not None:
                        student[1].boundary_weights.grad[:args.num_seen_classes] = 0
...
if __name__ == "__main__":
    ...
    # 在 train_happy.py 的参数解析部分添加
    parser.add_argument('--use_adbs', action='store_true', default=True,
                        help='Use Adaptive Decision Boundary Strategy')
    parser.add_argument('--adbs_alpha', type=float, default=0.05,
                        help='Weight for inter-class constraint loss')
    ...
    # ----------------------
        ...
        for session in range(start_session, args.continual_session_num): # cifar100: session 0->4
            ...
            args.logger.info('transferring boundary weights of seen classes...')
            projector_cur.boundary_weights.data[:args.num_seen_classes] = projector_pre.boundary_weights.data[
                                                                          :args.num_seen_classes].clone()

            # 为新类别初始化边界权重（使用旧类别边界的平均值）
            if args.num_seen_classes > 0:
                mean_boundary = projector_pre.boundary_weights.data[:args.num_seen_classes].mean()
                projector_cur.boundary_weights.data[args.num_seen_classes:] = mean_boundary
                args.logger.info(f'New classes boundary weights initialized with mean value: {mean_boundary:.4f}')
            else:
                # 第一个session，所有边界权重已经在DINOHead初始化为1
                args.logger.info('First session: all boundary weights initialized to 1.0')
