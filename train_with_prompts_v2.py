...
from models.enhanced_backbone import create_enhanced_backbone, get_enhanced_params_groups
...
def create_enhanced_optimizer(param_groups, base_lr, momentum=0.9, weight_decay=5e-5):
    """
    创建支持不同学习率的优化器
    """
    # 处理prompts参数的学习率缩放
    ...
    for group in param_groups:
        ...
        if 'lr_scale' in group:
            # 应用学习率缩放
            lr_scale = new_group.pop('lr_scale')
            new_group['lr'] = base_lr * lr_scale
            print(f"Prompts parameters learning rate: {new_group['lr']}")
        else:
            new_group['lr'] = base_lr
        processed_groups.append(new_group)

    optimizer = SGD(processed_groups, lr=base_lr, momentum=momentum, weight_decay=weight_decay)
    return optimizer


def train_offline(student, prompts_enhancer, train_loader, test_loader, args):
    """离线训练函数 - 修改以支持端到端prompts训练"""

    # 获取参数组
    params_groups = get_enhanced_params_groups(student, prompts_enhancer,
                                               prompts_lr_scale=args.prompts_lr_scale)

    optimizer = create_enhanced_optimizer(params_groups, args.lr, args.momentum, args.weight_decay)
    ...
    for epoch in range(args.epochs_offline):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            mask_lab = torch.ones_like(class_labels)  # 离线训练所有样本都有标签

            class_labels, mask_lab = class_labels.cuda(non_blocking=True), mask_lab.cuda(non_blocking=True).bool()
            images = torch.cat(images, dim=0).cuda(non_blocking=True)

            # 使用增强的backbone（内部处理prompts）
        ...
        # 保存prompts状态
        if prompts_enhancer is not None:
            prompts_save_path = args.model_path.replace('.pt', '_prompts.pt')
        ...
        if old_acc_test > best_test_acc_old:
            ...
            # 保存最佳prompts状态
            if prompts_enhancer is not None:
                best_prompts_save_path = args.model_path[:-3] + f'_best_prompts.pt'
...
def train_online(student, student_pre, proto_aug_manager, prompts_enhancer, train_loader, test_loader, current_session,
    ...
    for epoch in range(args.epochs_online_per_session):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            # 获取当前模型的基础特征（不通过prompts增强）
            if hasattr(student[0], 'base_vit'):
                # 如果是增强的backbone，获取基础特征用于蒸馏
                feats = student[0].base_vit(images)
            ...
            # 特征蒸馏损失（使用基础特征，不受prompts影响）
            feat_distill_loss = (feats - feats_pre).pow(2).sum() / len(feats)

            # Prompts损失 - 包含attention_info用于更精确的损失计算
            prompts_loss = torch.tensor(0.0, device=images.device)
            if prompts_enhancer is not None:
                # 获取当前batch的attention信息
                with torch.no_grad():
                    # 模拟获取attention信息（在实际的enhanced backbone中会自动计算）
                    cls_tokens = torch.cat([x[:1] for x in torch.split(feats.unsqueeze(1), 1, dim=0)], dim=0)
                    temp_x = torch.cat([cls_tokens, feats.unsqueeze(1)], dim=1)
                    _, attention_info = prompts_enhancer(temp_x, return_attention=True)

                prompts_losses = prompts_enhancer.compute_prompts_losses(attention_info)
                prompts_loss = prompts_losses['total']
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Enhanced Continual GCD with End-to-End Prompts',
    ...
    parser.add_argument('--use_prompts_enhancement', action='store_true', default=True,
                        help='Whether to use enhanced prompts')
    ...
    parser.add_argument('--prompts_top_k', type=int, default=5,
                        help='Number of top prompts to use')
    parser.add_argument('--prompts_lr_scale', type=float, default=0.1,
                        help='Learning rate scale for prompts parameters')
    ...
    parser.add_argument('--exp_name', default='enhanced-continual-gcd', type=str)
    ...
    args.exp_name = 'Enhanced_CGCD' + '-' + args.train_session
    ...
    init_experiment(args, runner_name=['Enhanced_Continual_GCD'])
    ...
    args.logger.info('Base model built')

    # 创建Enhanced PromptsEnhancer
    prompts_enhancer = None
    if args.use_prompts_enhancement and args.prompts_pool_path is not None:
        args.logger.info('Initializing Enhanced PromptsEnhancer...')
        try:
            ...
            args.logger.info('Enhanced PromptsEnhancer initialized successfully!')
            args.logger.info(f'Prompts statistics: {prompts_enhancer.get_statistics()}')
        except Exception as e:
            args.logger.error(f'Failed to initialize Enhanced PromptsEnhancer: {str(e)}')
            args.logger.info('Continuing without prompts enhancement...')
            prompts_enhancer = None
    else:
        args.logger.info('Enhanced prompts enhancement disabled')

    # 创建增强的backbone
    enhanced_backbone = create_enhanced_backbone(backbone, prompts_enhancer)

    # 投影头
    projector = DINOHead(in_dim=args.feat_dim, out_dim=args.mlp_out_dim, nlayers=args.num_mlp_layers)
    model = nn.Sequential(enhanced_backbone, projector)
    model.to(device)

    args.logger.info('Enhanced model with prompts support created')
    ...
    # 训练流程
    if args.train_session == 'offline':
        args.logger.info('========== Enhanced offline training with end-to-end prompts ==========')
        ...
        with open(save_dataset_dict_path, 'w') as f_dataset_dict:
        ...
        # 训练
        train_offline(model, prompts_enhancer, offline_session_train_loader, offline_session_test_loader, args)

    elif args.train_session == 'online':
        args.logger.info(
            '\n\n========== Enhanced online continual GCD with end-to-end prompts ==========')
        args.logger.info('loading dataset...')
        ...
        for session in range(start_session, args.continual_session_num):
            args.logger.info(
                '\n\n========== begin enhanced online continual session-{} ==============='.format(session + 1))
            ...
            # 设置可训练参数
            if hasattr(enhanced_backbone, 'base_vit'):
                for m in enhanced_backbone.base_vit.parameters():
                    m.requires_grad = False

                for name, m in enhanced_backbone.base_vit.named_parameters():
                    if 'block' in name:
                        block_num = int(name.split('.')[1])
                        if block_num >= args.grad_from_block:
                            m.requires_grad = True
            ...
            if session == 0:
                ...
                # 创建上一会话的增强backbone
                backbone_pre = vits.__dict__['vit_base']()
                backbone_pre.load_state_dict(enhanced_backbone.base_vit.state_dict())
                enhanced_backbone_pre = create_enhanced_backbone(backbone_pre, prompts_enhancer)
                model_pre = nn.Sequential(enhanced_backbone_pre, projector_pre)
            ...
            # 创建当前会话的增量分类器
            backbone_cur = deepcopy(enhanced_backbone.base_vit)
            enhanced_backbone_cur = create_enhanced_backbone(backbone_cur, prompts_enhancer)
            ...
            if args.init_new_head:
                new_head = get_kmeans_centroid_for_new_head(model_pre, prompts_enhancer,
            ...
            model_cur = nn.Sequential(enhanced_backbone_cur, projector_cur)
            ...
            if session == 0:
                ...
                offline_session_train_loader_for_proto_aug = DataLoader(offline_session_train_dataset_for_proto_aug,
                                                                        num_workers=args.num_workers_test,
                                                                        batch_size=256, shuffle=False, pin_memory=False)
                proto_aug_manager.update_prototypes_offline(model_pre, prompts_enhancer,
            ...
            proto_aug_manager.update_prototypes_online(model_cur, prompts_enhancer,
                                                       online_session_train_loader_for_new_head_init,
                                                       args.num_seen_classes,
        ...
        # 打印所有会话的最终结果
        args.logger.info(
            '\n\n==================== Enhanced CGCD Final Results over {} continual sessions ===================='.format(
