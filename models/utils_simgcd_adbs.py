...
class DINOHead(nn.Module):
    def __init__(self, in_dim, out_dim, use_bn=False, norm_last_layer=True, 
        ...
        # 添加ADBS边界权重
        self.boundary_weights = nn.Parameter(torch.ones(out_dim))
    ...
    def forward(self, x):
        ...
        logits = self.last_layer(x) * self.boundary_weights.unsqueeze(0)

        return x_proj, logits


class InterClassConstraintLoss(nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def forward(self, model, prototypes):
        ...
        for i in range(num_classes):
            for j in range(num_classes):
                if i != j:
                    # 计算 (1 - mi)pi^T wi + (mj - 1)pi^T wj
                    pi_wi = torch.dot(prototypes[i], classifier_weights[i])
                    pi_wj = torch.dot(prototypes[i], classifier_weights[j])

                    constraint_value = (1 - boundary_weights[i]) * pi_wi + (boundary_weights[j] - 1) * pi_wj
