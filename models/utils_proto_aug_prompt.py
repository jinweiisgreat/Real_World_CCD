...
class ProtoAugManager:
    ...
    def update_prototypes_online(self, model, train_loader, num_seen_classes, num_all_classes):
        ...
        for batch_idx, (images, label, _, _) in enumerate(tqdm(train_loader)):
            images = images.cuda(non_blocking=True)
            with torch.no_grad():
                _, logits = model(images)  # 使用PromptEnhancedModel的forward
                feats = model.backbone(images)  # 直接获取backbone输出
                feats = torch.nn.functional.normalize(feats, dim=-1)
                all_feats_list.append(feats)
        ...
        for c in range(num_seen_classes, num_all_classes):
            ...
            if len(feats_c) == 0:
                ...
                feats_c_mean = model.projector.last_layer.weight_v.data[c]  # 直接访问projector
            else:
