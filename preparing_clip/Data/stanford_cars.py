...
class CarsDataset(Dataset):
    ...
    def __init__(self, train=True, limit=0, data_dir=car_root, transform=None):
        ...
        for idx, img_ in enumerate(labels_meta['annotations'][0]):
            ...
            self.data.append(data_dir + img_[5][0]) # _jinwei/Dataset/下的数据集不用
            # print(f'data_dir +img_[5][0]:{data_dir +'0'+img_[5][0]}')#jpg名字
            # self.data.append(data_dir + img_[5][0])
            # if self.mode == 'train':
            self.target.append(img_[4][0][0])#class
            # print(f'img_[4][0][0]:{img_[4][0][0]}')
