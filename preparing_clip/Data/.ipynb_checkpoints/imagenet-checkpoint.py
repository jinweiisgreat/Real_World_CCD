...
def get_imagenet_100_datasets(train_transform, test_transform, train_classes=range(80),
    ...
    # dict_data=json.loads(data).replace("'", "\"")#转化为dict 数据
    ...
    # print(f'subsampled_100_classes:{subsampled_100_classes}')
    cls_map = {i: j for i, j in zip(subsampled_100_classes, range(100))}#给类名赋予下标
    ...
    # print(f'whole_training_set:{whole_training_set}')
    # image, label = whole_training_set[0] 
    # print(f'whole_training_set.classes :{whole_training_set.classes}')
    # print(f'whole_training_set.imgs :{whole_training_set.imgs}')#所有的img文件路径，格式为('路径',label)
    
    # print(f'len(whole_training_set[0]):{len(whole_training_set[0])}')
    # print(f'len(whole_training_set[0][0]):{len(whole_training_set[0][0])}')
    # samples_ = []
    # for i in range(len(subsampled_100_classes)):
    #     samples_.append(whole_training_set.samples[i])
    # whole_training_set.samples = samples_
