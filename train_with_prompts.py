...
def get_optimizer_with_prompts(model, prompts_enhancer, args):
    """
    获取包含prompts参数的优化器
    """
    # 原有的模型参数分组
    ...
    # 如果有prompts enhancer，添加其参数
    if prompts_enhancer is not None:
        prompts_params = prompts_enhancer.get_prompts_parameters()
        if prompts_params:
            # 将prompts参数作为一个新的参数组添加
            prompts_group = {'params': prompts_params, 'lr': args.lr * 0.1}  # 使用较小的学习率
...
def train_online(student, student_pre, proto_aug_manager, prompts_enhancer, train_loader, test_loader, current_session,
                 args):
    optimizer = get_optimizer_with_prompts(student, prompts_enhancer, args)
    ...
    for epoch in range(args.epochs_online_per_session):
        ...
        # 打印prompts统计信息
        if prompts_enhancer is not None and epoch % 10 == 0:
